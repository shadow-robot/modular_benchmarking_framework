...
#include <modular_framework_core/GraspCommand.h>
#include <modular_framework_core/GraspAction.h>
#include <modular_framework_core/GraspFeedback.h>
...
class GraspActionServer
{
  private:
    ...
    actionlib::SimpleActionServer<modular_framework_core::GraspAction> action_server_;
  ...
  public:
    // Constructor of the class that takes as input a ROS node handler, the name of the action server
    // and the name of the manipulator group.
  ...
  private:
    ...
    void goal_callback()
    {
        ...
        std::transform(current_joint_values.begin(), current_joint_values.end(), current_joint_values.begin(),
                       [](double& value) { return std::roundf(value); });
    ...
    // Method called everytime a preempt order is received.
    // It stops an execution if one is running and set the action server to preempted
...
int main(int argc, char** argv)
{
    ...
    node_handler.getParam("action_server_name", action_server_name);
