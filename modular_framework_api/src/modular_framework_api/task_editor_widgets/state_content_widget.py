...
class GenericConfigBoxWidget(QGroupBox):
    ...
    def get_text_from_slot(self, slot_name):
        """
            Get the current text set to a given slot

            @param slot_name: String specifying from which slot we want the text
            @return: String corresponding to the current text
        """
        return self.findChild(QLineEdit, "line {}".format(slot_name)).text()

    def set_text_to_slot(self, slot_name, value):
        """
            Set some text to a given slot

            @param slot_name: String specifying for which slot we want to set the text
            @param value: String to set to the corresponding slot
        """
        self.findChild(QLineEdit, "line {}".format(slot_name)).setText(value)
